    ...
    def act(self, game):
        def Get_Action_For(self_bot, game, depth):            
            ...
            def places_to_go(from_loc, occupied_space_list):
                ret_list = []
                nearby_places = rg.locs_around(loc , filter_out=('invalid', 'obstacle'))
                if(game['turn']%10 == 0 or game['turn']%10 == 9):
                    nearby_places = rg.locs_around(loc , filter_out=('invalid', 'obstacle', 'spawn'))
                for targ in nearby:
            ...
            if(bots_attacking_me > 1.6): #try to run away or suicide
      
                if(self_bot.hp <= min_attack):
                    return ['suicide']
                else:
                    for targ in rg.locs_around(self_bot.location , filter_out=('invalid', 'obstacle')):
                        if not occupied(targ, occupied_locations):
                            nearby_enemy = False
                    ...
                    if(self_bot.hp <= bots_attacking_me*min_attack):
                        return ['suicide']
            if(self_bot.hp < min_attack and adjacent_enemies >=2):
                return ['suicide']
            ...
            if(self_bot.location != rg.CENTER_POINT):
                return ['move', rg.toward(self_bot.location, rg.CENTER_POINT)]
            else:
                return ['guard']
